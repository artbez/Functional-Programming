...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences =  (w.groupBy((element: Char) => element.toLower) map (x => (x._1, x._2.length))).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def addOcc(acc: Occurrences, cur: Word): Occurrences = {
      val accMap = (acc map (x => x._1 -> x._2) toMap) withDefaultValue 0
      val currMap = wordOccurrences(cur) map (x => x._1 -> x._2) toMap
      val res = for (p <- currMap) yield (p._1 -> (p._2 + accMap(p._1)))
      (accMap ++ res).toList.sorted
    }
    s.foldLeft(List[(Char, Int)]()) (addOcc)
  }
